...
public class BossSlime : GroundEnemy
{
    ...
    [SerializeField] private int deathStateLayer = 0;
    ...
    [SerializeField] private int phase3StateLayer = 0;           // 재생할 레이어 인덱스(보통 0)
    ...
    private SpriteRenderer _sr;          // 스프라이트 숨김용

    private Collider2D[] _hurtCols;
    ...
    [SerializeField] private Color skill3SlamBoxFill = new Color(1f, 0.3f, 0.2f, 0.08f);
    ...
    [SerializeField] private Color skill4BoxLineColor = new Color(1f, 0.4f, 0.2f, 0.9f);
    [SerializeField] private Color skill4BoxFillColor = new Color(1f, 0.4f, 0.2f, 0.08f);
    ...
    public bool IsInAttack => StateMachine != null && StateMachine.CurrentState is BossAttackState;

    public bool IsAnyBossSkillRunning => _bossAttackState != null && _bossAttackState.IsSkillRunning;
    ...
    protected override void Awake()
    {
        ...
        _sr = SpriteRendererComponent;

        var allCols = GetComponentsInChildren<Collider2D>(true);
        _hurtCols = System.Array.FindAll(allCols, c => c != null && c.isTrigger);
        ...
        _bossAttackState.SetAllowsMovement(SkillType.Skill3, true);
    ...
    private bool IsGroundedFast()
    {
        ...
        return MovementComponent != null &&
               MovementComponent.IsGround != null &&
               MovementComponent.IsGround.Value;
    }
    ...
    protected override void Update()
    {
        ...
        bool inAttack = StateMachine != null && StateMachine.CurrentState is BossAttackState;
    ...
    private void ResetAllTriggersExcept(params string[] allow)
    {
        ...
        for (int i = 0; i < ps.Length; i++)
        {
            ...
            for (int k = 0; k < allow.Length; k++)
                if (p.name == allow[k]) { keep = true; break; }
    ...
    {
        if (_hurtCols == null) return;
        for (int i = 0; i < _hurtCols.Length; i++)
            if (_hurtCols[i]) _hurtCols[i].enabled = !v;  // 무적이면 끔
    }
    ...
    private void SetTriggerOnTargets(string trigger)
    {
        if (string.IsNullOrEmpty(trigger) || phase3VanishTargets == null) return;
        foreach (var go in phase3VanishTargets)
        {
            if (!go || !go.activeInHierarchy) continue;
            var anims = go.GetComponentsInChildren<Animator>(true);
            foreach (var a in anims) a.SetTrigger(trigger);
        }
    }
    private void OnBossHit()
    {
        ...
        if (hp != null)
        {
            Debug.Log($"[BossSlime] Hit! HP {hp.CurrentHealth}/{hp.MaxHealth} (norm={hp.NormalizedHealth:F3})");
    ...
    private void OnBossDeath()
    {
        ...
        Debug.Log("[BossSlime] Boss Dead! (force death anim)");
    ...
    private DamageCaster FindCasterByName(string wantedName)
    {
        if (string.IsNullOrEmpty(wantedName)) return null;
        if (_damageCasters == null || _damageCasters.Length == 0)
            _damageCasters = GetComponentsInChildren<DamageCaster>(true);
        foreach (var dc in _damageCasters)
            if (dc && dc.name == wantedName) return dc;
        return null;
    }
    ...
    private System.Collections.IEnumerator Phase3EntrySmart()
    {
        ...
        {
            Debug.Log("[BossSlime] Phase3 pending until landing...");
            // 점프/낙하가 끝날 때까지 기다림
    ...
    {
        Debug.Log("[BossSlime] Death pending until landing...");
        while (!IsGroundedFast())
            yield return null;
    ...
    {
        Debug.Log("[BossSlime] Play death sequence (grounded)");
